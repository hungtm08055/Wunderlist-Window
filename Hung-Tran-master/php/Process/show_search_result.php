<?php 
    ...
    $search = $db->prepare("SELECT * FROM task WHERE title LIKE  '%$search_input%'");
    $search->execute();

    while($result = $search->fetch(PDO::FETCH_ASSOC))
    {
        $task_listId = $db->prepare("SELECT * FROM task WHERE title = ?"); // lay listId cua bang list
        $task_listId->execute(array($result['title']));

        foreach($task_listId as $task_row)
        {
            if($result['id'] == $task_row['id'] && $task_row['status'] == 0) // so sánh 2 id từ bảng lisst và khóa ngoại listId của bảng task
            {
                                      
                echo '<div class="tasklist-body" id="tasklist-body1" condition=0 id_list='.$task_row['listId'].' id_task='.$task_row['id'].' status=1 unstatus=0 star=1 unstar=0 date_create='.$task_row['createdate'].'>'                                    
            ...
            if($result['id'] == $task_row['id'] && $task_row['status'] == 1)
            { 
                echo '<div class="task-list-Completed op">'
                . '<div class="tasklist-body" id="tasklist-body1" condition=1 id_list='.$task_row['listId'].' id_task='.$task_row['id'].' status=1 unstatus=0 star=1 unstar=0 date_create='.$task_row['createdate'].'>'
                    ...
                    // .'<a href="hienthi.php?id_task='.$task_row['id'].'&id_list='.$list_id.'">'; 
                        echo '<div id="task-completed-text">';
                            echo $result['title']; // hiển thị title của task
