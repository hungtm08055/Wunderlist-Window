<?php 
    ...
    $task_id = $_POST['id_task'];   //from ajax
    $list_id = $_POST['id_list'];   //from ajax

    if(isset($task_id))
    {
        $task = $db->prepare("SELECT * FROM task WHERE id=$task_id"); 
        $task->execute();
        ...
        {                                                 
            ...
            {
                echo '<div class="status-father" id_list='.$task_row['listId'].' id_task='.$task_row['id'].' status=1 unstatus=0 star=1 unstar=0>'
                    ...
                    .'</div>'
                    .'<form action="../php/Process/update-Task.php" method="POST" id="edit_task">'
                    .'<input class="task_input_text" name="task_input" type="text" value="'.$task_row['title'].'">' // echo ra title của task có id_task tương ứng 
                    .'</form>'
                . '</div>';
            }                                                                                                           
        }
